...
def format_for_nn_training_singlestream_2d(imgs_np: list):
    """
    ...
    for cur_stream_idx in range(tot_num_streams):
        ...
        for cur_batch_idx in range(batch_size):
            input_single_batch.append(np.expand_dims(imgs_np[cur_batch_idx][cur_stream_idx], axis=2))
        input_single_batch = np.array(input_single_batch)
        nn_array.append(input_single_batch)
    return nn_array


def format_for_nn_training_singlestream(imgs_np: list):
    """
    From a list of numpy arrays, it generates the proper NN output.
    For example. If input is a 3 mult-stream network with batch of 2: [#_streams,batch_size,width,height,depth,bands]
    :param imgs_np:
    ...
    for cur_stream_idx in range(tot_num_streams):
        ...
        # Iterate each image = stream
        for cur_batch_idx in range(batch_size):
            # Swap wasn't needed before, need to review it
            # input_single_batch.append(np.expand_dims(np.swapaxes(imgs_np[cur_batch_idx][cur_stream_idx], 0,2), axis=3))
            input_single_batch.append(np.expand_dims(imgs_np[cur_batch_idx][cur_stream_idx], axis=3))
        input_single_batch = np.array(input_single_batch)
        nn_array.append(input_single_batch)

    return nn_array

def format_for_nn_training_multistream(imgs_np: list):
    """
    From a list of numpy arrays, it generates the proper NN output.
    For example. If input is a 3 mult-stream network with batch of 2: [#_streams,batch_size,width,height,depth,bands]
    :param imgs_np:
...
def format_for_nn_classification(imgs_np: list):
    ...
    for cur_img_idx in range(tot_num_streams):
        # Swap wasn't needed before, need to review it
        # temp = np.expand_dims(np.expand_dims(np.swapaxes(imgs_np[cur_img_idx],0,2), axis=0), axis=4)
