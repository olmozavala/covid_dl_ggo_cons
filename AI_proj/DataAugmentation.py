...
def shifting_3d_single(imgs, ctrs, max_shift_perc=.1):
    ...
    if shiftSize != 0:
        for i in range(len(imgs)):
            imgs[i, :, :, :] = utils.shift3D(imgs[i, :, :, :], shiftSize, shiftAxis)
        for i in range(len(ctrs)):
            ctrs[i, :, :, :] = utils.shift3D(ctrs[i, :, :, :], shiftSize, shiftAxis)
...
def shifting_2d(imgs, ctrs, max_shift_perc=.1):
    shiftSize = int(imgs[0].shape[1] * max_shift_perc * (np.random.random() * 2 - 1))
    shiftAxis = np.random.randint(0, 2)
    # print("Shifting {} Axis {}".format(shiftSize, shiftAxis))
    if shiftSize != 0:
        for i in range(len(imgs)):
            imgs[i, :, :] = utils.shift2D(imgs[i, :, :], shiftSize, shiftAxis)
        for i in range(len(ctrs)):
            ctrs[i, :, :] = utils.shift2D(ctrs[i, :, :], shiftSize, shiftAxis)
